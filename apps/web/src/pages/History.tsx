      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display font-bold text-2xl uppercase tracking-tight text-text-primary">
            Conversion <span className="text-brand">History</span>
          </h1>
          <p className="text-xs text-text-secondary">Your past converted slips and booking codes</p>
        </div>
      </div>
      ...
      {history.length === 0 ? (
        <Card className="flex flex-col items-center justify-center py-12 text-center gap-3">
          ...
          <p className="text-xs text-text-secondary max-w-xs">
            Any booking code you convert will be saved here for quick re-copying.
          </p>
        </Card>
      ) : (
        <div className="flex flex-col gap-3">
          {history.map((item) => (
            <Card key={item.id} className="flex flex-col gap-2">
              ...
              {item.dest_code && (
                <div className="flex items-center justify-between bg-surface-subtle p-2.5 rounded-xl border border-border-subtle mt-1">
